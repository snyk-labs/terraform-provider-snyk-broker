...
var _ = Describe("BrokerDeploymentContextsDataSource", func() {
	var (
		d        *BrokerDeploymentContextsDataSource
	...
	Describe("Metadata", func() {
		It("returns the correct type name", func() {
			...
			Expect(resp.TypeName).To(Equal("snyk_broker_deployment_contexts"))
		})
	})
	...
	Describe("Configure", func() {
		It("sets the client from provider data", func() {
			...
			newDS := &BrokerDeploymentContextsDataSource{}
	...
	Describe("Client API calls", func() {
		Context("ListDeploymentContexts", func() {
			It("lists all contexts for a deployment", func() {
				...
				result, err := d.client.ListDeploymentContexts(ctx, "tenant-123", "install-456", "deploy-789")
			...
			It("returns an empty list for deployment with no contexts", func() {
				mockHTTP.DoFunc = func(req *http.Request) (*http.Response, error) {
					return testutil.NewMockJSONResponse(http.StatusOK, `{"data": []}`), nil
				}

				result, err := d.client.ListDeploymentContexts(ctx, "tenant-123", "install-456", "empty-deploy")
				...
				mockHTTP.DoFunc = func(req *http.Request) (*http.Response, error) {
					...
					if callCount == 1 {
						return testutil.NewMockJSONResponse(http.StatusOK, `{
							...
							"links": {
								"next": "https://api.snyk.io/rest/tenants/tenant-123/brokers/installs/install-456/deployments/deploy-789/contexts?starting_after=ctx-1"
							}
						}`), nil
					}
